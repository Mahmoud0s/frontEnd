...
function checkUser() {
    if (!Cookies.get("token")) location.href = "/login";
    else return true;
}
...
const hallsNum = document.querySelector("#hallsNum");
...
async function showAnalytics(){
    ...
    // get halls number 
    res= await fetch("http://localhost:5000/api/halls/all",{
        headers: {
                    "Content-Type": "application/json",
                    'Authorization': Cookies.get('token')
            },
    ...
    if(res.status==401 || res.status==403){
        ...
        let data=await res.json()
        hallsNum.textContent=data.length
    }
}
